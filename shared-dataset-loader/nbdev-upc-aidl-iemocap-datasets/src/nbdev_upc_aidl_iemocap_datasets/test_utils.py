...
def create_test_fixtures() -> tuple[tempfile.TemporaryDirectory, Path]:
    ...
    manifest_data = [
        ...
        },
        {
            ...
            "last_export_filename": (
                "http://example.com/dataset_audio_chunks/"
                "group_id_8__last_export_1779999999.parquet"
            ),
            "last_export_at": "2026-05-18 22:01:23.000000",
        },
    ]
    ...
    def _parquet_data(dataset_audio_chunk_group_id: str):
        if dataset_audio_chunk_group_id == '8':
            return [
                {**_common, 'id': 16, 'dataset_audio_chunk_group_id': '8',
                'source_dataset_table_name': 'external_audio_source',
                'source_dataset_row_index': 12,
                'speaker_id': 'test.wav',
    ...
    pd.DataFrame(_parquet_data(dataset_audio_chunk_group_id='8')).to_parquet(
        tests_path / 'group_id_8__last_export_1779999999.parquet'
    )
    ...
    # ── 4. External SQLite database ──────────────────────────────────────────
    ext_db_path = tests_path / DatasetsFactory.EXTERNAL_DB_FILE_NAME
    ext_conn = sqlite3.connect(ext_db_path)
    ext_conn.execute("""
        CREATE TABLE external_audio_source (
            source_dataset_row_index INTEGER,
            audio_bytes              BLOB
        )
    """)
    ext_conn.execute("""
        INSERT INTO external_audio_source VALUES (?, ?)
    """, (12, audio_bytes))
    ext_conn.commit()
    ext_conn.close()
